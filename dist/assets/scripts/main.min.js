(function () {
  ...
  class Navbar {
    ...
    init() {
      ...
      this.element.addEventListener('click', _ref => {
        let {
          target
        } = _ref;
  ...
  class Popup {
    constructor() {
      let selector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '.js-popup';
  ...
  class Quiz {
    constructor() {
      let selector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '.js-quiz';
      this.element = document.querySelector(selector);
      if (this.element) {
    ...
    updateResultValue() {
      ...
      const result = this.stepRatios.every(ratio => !isNaN(Number(ratio))) ? this.stepRatios.reduce((acc, ratio) => acc * ratio, 1) : null;
      this.resultValue.textContent = result ? `${Number.parseFloat(result).toFixed(2)} ₽` : 'Не удалось определить сумму...';
    }
  ...
  class Review {
    constructor() {
      let selector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '.js-review';
      let {
        collapsed = true,
        contentLength = 100,
